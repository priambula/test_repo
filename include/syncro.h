...
/// @class TCustomLock
/// @brief Класс авторазблоикировки Аналог unque_lock из mutex.h
template<typename _Mutex>
class TCustomLock
{
public:
  typedef _Mutex mutex_type;
    /// Конструктор класса авторазблоикировки
    explicit TCustomLock(mutex_type& __m) noexcept
    : m_device(&__m), m_owns(false)
    { }
    /// Деструктор класса авторазблоикировки
    ~TCustomLock()
    {
        if (m_owns)
        {
...
private:
    /// Указатель на блокировщик
    mutex_type*	m_device = nullptr;
    /// Флаг владения блокировщиком
    bool		m_owns; // XXX use atomic_bool
};

/// @struct _TOptimSpinLock
/// @brief Класс спин-блоикировки на основе атомарной переменной
struct _TOptimSpinLock
{   
    inline static void initlock(std::atomic_flag &_locked) noexcept
    {
...
/// @struct TOptimRWLock
/// @brief Класс блоикировки чтения-записи на основе атомарной переменной
...
/// Получения счётчика тактов проессора
