...
TMultiPipeProcessor<TValue,TcurrContainer > * tb_pmpp;
                   ...
                   TB_TThreadArgs & args,TThreadArgs & _args)
{
    ...
    {        
        ...
        tb_th_timelist[0] = clock::now();
        tb_th_timelist[1] = clock::now();
        tb_th_timelist[2] = clock::now();
        tb_timestamp = clock::now();
        tb_clkstamp = getRDTDC();

        for (auto i1 = 0; i1 < N_PRODUCERS; i1++)
        {
            tb_th_keylist[i1].push_back(tb_keylist[0]);
            tb_th_keylist[i1].push_back(tb_keylist[1]);
            tb_th_keylist[i1].push_back(tb_keylist[2]);
...
void readListFromFile()
{
    ...
    if (istrstm != nullptr)
    {
        while (getline((*istrstm),lexema,c_columndelimiter1) != 0)
        {
            if (lexema.empty())
            {
               continue;
